...
import type { Department, School } from '@/lib/types/models';
...
export default function AdminDepartmentsPage() {
    ...
    const [formData, setFormData] = useState<CreateDepartmentData>({ school_id: 0, code: '', name: '', description: '', is_active: true });
    ...
    const openCreateForm = () => {
        setFormMode('create');
        setEditingDept(null);
        setFormData({ school_id: schools.length > 0 ? schools[0].id : 0, code: '', name: '', description: '', is_active: true });
        setFieldErrors({});
        setErrorMessage('');
                            ...
                            <div className="grid gap-4 sm:grid-cols-2">
                                <div className="space-y-2">
                                    ...
                                    <select
                                        ...
                                        required
                                    >
                                        <option value={0} disabled>Select a school</option>
                                        {schools.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                                    </select>
                                    {fieldErrors['school_id'] && (
                                        <p className="text-sm font-medium text-destructive">{fieldErrors['school_id'][0]}</p>
                                    )}
                                </div>
                                <div className="space-y-2">
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b bg-muted/50">
                                    ...
                                    <th className="px-4 py-3 text-left font-medium text-muted-foreground hidden lg:table-cell">School</th>
