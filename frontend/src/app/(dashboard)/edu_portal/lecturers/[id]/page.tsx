...
import type { User, School, Department } from '@/lib/types/models';
...
import {
    ...
    Mail, Phone, Calendar, UserCheck, UserX, Hash, Building2
} from 'lucide-react';
...
export default function LecturerDetailPage() {
    ...
    const fetchUser = useCallback(async () => {
        ...
        try {
            ...
            setGeneralError('Failed to load lecturer. They may not exist.');
        } finally {
            setIsLoading(false);
        }
    }, [userId]);
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            const payload: UpdateUserData = { ...form, role: 'lecturer' };
            if (!payload.password) {
            ...
            setUser(res.data.user);
            setSuccessMessage(res.message || 'Lecturer updated successfully');
            setIsEditing(false);
        } catch (err: unknown) {
            ...
            setGeneralError(error.response?.data?.message || 'Failed to update lecturer.');
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            router.push('/edu_portal/lecturers');
        } catch (err) {
            console.error('Delete failed:', err);
            setGeneralError('Failed to delete lecturer.');
            ...
            <div className="space-y-4">
                ...
                <Card>
                    <CardContent className="py-16 text-center">
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">Lecturer not found or has been deleted.</p>
                    </CardContent>
                </Card>
            </div>
        );
                    ...
                    <Card className="md:col-span-2">
                        ...
                        <CardContent>
                            <dl className="grid gap-6 sm:grid-cols-2">
                                ...
                                <InfoRow icon={<Hash className="h-4 w-4" />} label="File Number" value={user!.staff_id || '—'} />
                                ...
                                />
                                {user!.school && (
                                    <InfoRow icon={<Building2 className="h-4 w-4" />} label="School"
