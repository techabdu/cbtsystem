...
import {
    ...
    ChevronDown, ChevronUp, RotateCcw, Users,
} from 'lucide-react';

const COLUMNS = [
    { column: 'Full Name',       required: true,  example: 'John Doe',         notes: 'First + Last name (space-separated)' },
    { column: 'Identifier',      required: true,  example: 'STF/2024/001',     notes: 'Staff file number — lecturer uses this to activate their account' },
    { column: 'Email',           required: false, example: 'john@college.edu', notes: 'Optional — placeholder auto-generated if left blank' },
    { column: 'Role',            required: true,  example: 'lecturer',         notes: 'Must be exactly "lecturer" (lowercase)' },
    { column: 'Department Code', required: false, example: 'CSC',              notes: 'Department code — school is auto-assigned from this' },
    { column: 'Phone',           required: false, example: '+2348012345678',   notes: 'Optional contact number' },
];

type UploadState = 'idle' | 'uploading' | 'done';

export default function BulkUploadLecturersPage() {
        ...
        <div className="space-y-6 max-w-3xl">
            ...
            <div>
                <nav className="flex items-center gap-1 text-sm text-muted-foreground mb-2">
                    <Link href="/edu_portal/lecturers" className="hover:text-foreground transition-colors">
                        Lecturers
                    </Link>
                    <ChevronRight className="h-4 w-4" />
                    <span className="text-foreground font-medium">Bulk Upload</span>
                </nav>
                <h1 className="text-2xl font-bold tracking-tight">Bulk Upload Lecturers</h1>
                <p className="text-muted-foreground">
                    Upload an Excel spreadsheet to create multiple lecturer accounts at once.
                </p>
            </div>
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Download the Excel template, fill in the lecturer details, then upload the file below.
                        The <strong>Department Code</strong> (e.g. <code>CSC</code>) automatically assigns the lecturer's
                ...
                <CardContent className="space-y-4">
                    <Button
                        onClick={async () => {
                            ...
                            try { await downloadTemplate('bulk-lecturers-template'); }
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid gap-3 sm:grid-cols-3">
                            <div className="rounded-lg border bg-green-50 dark:bg-green-950/20 p-4 text-center">
                                ...
                                <p className="text-xs text-muted-foreground">Lecturers Created</p>
                            </div>
