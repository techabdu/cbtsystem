...
import { getDepartmentExams, type OfficerExamFilters } from '@/lib/api/officer';
...
export default function DepartmentExamsPage() {
    ...
    const hasAccess = Boolean(user?.is_department_exam_officer);
    ...
    const fetchExams = useCallback(async () => {
        ...
        try {
            ...
            const res = await getDepartmentExams(filters);
            setExams(res.data);
            setPagination(res.pagination);
        } catch (err: unknown) {
            const error = err as { response?: { data?: { message?: string } } };
            setFetchError(error.response?.data?.message || 'Failed to load department exams.');
        } finally {
            setIsLoading(false);
        }
                    ...
                    <CardContent className="pt-8 pb-8 text-center space-y-4">
                        ...
                        <div>
                            ...
                            <p className="mt-1 text-sm text-muted-foreground">
                                This page is restricted to Department Exam Officers. Contact your administrator
    ...
    return (
        <div className="space-y-6">
            {/* Page Header */}
            <div>
                <h1 className="text-2xl font-bold tracking-tight">Department Exams</h1>
                <p className="text-muted-foreground">
                    Overview of all exams in your department.
                </p>
            </div>
            ...
            <div className="flex items-start gap-3 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 dark:border-blue-800/50 dark:bg-blue-950/30">
                ...
                <p className="text-sm text-blue-800 dark:text-blue-300">
                    You are viewing all exams in your department as <strong>Department Exam Officer</strong>.
                    You can monitor exam statuses and access results for all department courses.
                </p>
            </div>
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        All exams across courses in your department.
                    </CardDescription>
                </CardHeader>
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b bg-muted/50">
                                    ...
                                    <th className="px-4 py-3 text-left font-medium text-muted-foreground hidden md:table-cell">Lecturer</th>
                                    <th className="px-4 py-3 text-center font-medium text-muted-foreground">Status</th>
                                    <th className="px-4 py-3 text-center font-medium text-muted-foreground hidden lg:table-cell">Type</th>
                                        ...
                                        return (
                                            <tr key={exam.id} className="border-b transition-colors hover:bg-muted/50">
                                                ...
                                                <td className="px-4 py-3">
                                                    <p className="font-medium truncate max-w-[220px]">{exam.title}</p>
